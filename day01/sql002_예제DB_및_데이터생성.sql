-- 데이터베이스 생성 / 최초 DB 생성할 때는 master(기본)에서 생성해야 함
-- USE는 해당 DB를 사용하겠다는 의미
USE [master]; -- []는 데이터베이스와 컬럼명에만 사용

-- 마당 데이터베이스 생성
CREATE DATABASE Madang; -- C++과 동일하게 한 문장 끝은 ;(생략가능)
GO -- 여기까지 일단 한 번 실행

-- GO는 T-SQL 문은 아니지만 현재까지 명령어를 SQL Server에 보내서 실행시키는 역할 
-- 즉, 명령어를 묶어서 배치로 서버에 보내는 역할 
...
-- Book, Customer, Orders 데이터 생성 / DML
