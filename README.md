...
- Database 학습
    - 관계 데이터 모델
        - 무결성 제약조건
            - 슈퍼키 - 튜플을 유일한 값으로 구분 지을 수 있는 속성 또는 속성집합 (고객번호, 주민번호, 고객번호/고객이름, 고객번호/주소, 고객번호/이름/전화번호, ... 이름만은 안됨)
            ...
            - 기본키(Primary Key★) - 여러 후보키 중에서 하나를 선정하여 대표로 삼는 키 (고객번호)
                - 고려사항 - 고유한 값(**Unique**), NULL불가(**Not Null**), 최소 속성의 집합, 개인정보 등의 보안사항은 자제
            ...
            - 외래키(Foriegn Key★) - 기본키를 참조하여 사용하는 것
                ...
                - NULL과 중복을 허용(Not Null인 경우도 있음), 자기 자신의 기본키를 외래키로 사용할 수도 있음, 외래키가 기본키의 속성 중 하나가 될 수도 있음
            
            - 데이터 무결성(Integrity) - DB에 저장된 데이터의 일관성과 정확성을 지키는 것
                - 도메인 무결성 제약조건 - 데이터 타입, NOT NULL, 기본값, 체크 특성을 지키는 것
                - 개체 무결성 제약조건(기본키 제약조건) - 고유한 값(Unique)에 NOT NULL (값이 중복되어선 안되고, 빠져도 안됨)
                - 참조 무결성 제약조건(외래키 제약조건) - 부모의 키가 아닌 값은 사용할 수 없음 (외래키가 바뀔 때 기본키의 값이 아닌 것은 제약을 받음)
    ...
    - SELECT문
        - OUTER JOIN(외부조인)
            ...
            - 집합연산 - JOIN도 집합이지만, 속성별로 가로로 병합하기 때문에 집합이라 부르지 않음. 집합은 데이터를 세로로 합치는 것을 뜻함
                ...
                - 합집합(UNION, 진짜 많이 사용) - UNION(중복 제거), UNION ALL(중복 허용)
...
- 파이썬 SQL Server 연동 프로그래밍
    - Madang DB 관리 프로그램
        ...
        - DB연결 설정 - Oracle, MySQL 등은 설정이 없음. 구성관리자에서 TCP/IP로 접근을 허용하지 않으면 접속 안됨
            ...
            7. 적용 후에 SQL Server 서비스 > SQL Server (MSSQLSERVER) 더블클릭 후, **다시 시작** 버튼 클릭, 재시작 필요

            ![구성관리자](https://github.com/guswlrla/basic-database-2024/blob/main/images/db006.png)
...
- 파이썬 SQL Server 연동 프로그래밍
    ...
    - 실행화면
      ...
      https://github.com/guswlrla/basic-database-2024/assets/158007465/9822b159-a69d-4a09-bc1a-2b362970a46a
...
- 인덱스 예제
    ...
    <!-- md 주석. ![인덱스](https://github.com/guswlrla/basic-database-2024/blob/main/images/db008.png) -->
    <!-- html img태그가 이미지 사이즈 조정 가능 -->
    <img src="https://github.com/guswlrla/basic-database-2024/blob/main/images/db008.png" width="900">
...
- 정규화
    ...
    - BCNF정규형 - 함수 종속성 X -> Y가 성립할 때 모든 결정자X가 후보키(기본키가 될 수 있는 속성)이면
    - 보통 BCNF까지 정규화를 함
    - 4정규형(다치 종속성), 5정규형(조인 종속성, 무손실 분해)

- 실무실습(사용자, 권한 ...)
    1. DB관리자(SSMS)
        - DB관리자로 HR데이터베이스 생성, 관계를 설정
        - DB관리자로 HR DB를 사용할 사용자 계정을 생성, 필요한 권한 설정(권한 분리)
            - 추가 - sa의 비번을 잃어버리면, Windows 인증으로 로그인 후 
            - SSMS > 보안 > 로그인 > sa > 속성에서 비밀번호 변경 > SQL Server 인증으로 sa 비밀번호 입력 후 로그인
            - SSMS > 보안 > 로그인 > sa > 새 로그인 클릭
...
- 실무실습
    ...
    ![HR_ERD](https://github.com/guswlrla/basic-database-2024/blob/main/images/db009.png)
